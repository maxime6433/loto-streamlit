...
if uploaded_file:
    ...
    freq_df = pd.DataFrame({
        ...
        "% Tirages": [round((boule_counts.get(i, 0) / total_tirages * 100)) for i in range(1, 50)],
        "Dernière sortie": [derniere_sortie[i] for i in range(1, 50)]
    })
    freq_df["Absence (jours)"] = (df_filtré['date_de_tirage'].max() - freq_df["Dernière sortie"]).dt.days
    freq_df = freq_df.sort_values(by="Sorties", ascending=False)
    ...
    st.dataframe(freq_df.sort_values(by="Absence (jours)", ascending=False).head(10).reset_index(drop=True))

    st.subheader("📌 Recommandations")
    top = freq_df.head(5)["Numéro"].tolist()
    bottom = freq_df.sort_values(by="Sorties").head(5)["Numéro"].tolist()
    absents = freq_df.sort_values(by="Absence (jours)", ascending=False).head(5)["Numéro"].tolist()
    mix = top[:3] + bottom[:2]

    col1, col2, col3, col4 = st.columns(4)
    with col1:
        st.markdown("**Top fréquence**")
        st.success(", ".join(str(int(n)) for n in sorted(top)))
    with col2:
        st.markdown("**Moins sortis**")
        st.warning(", ".join(str(int(n)) for n in sorted(bottom)))
    with col3:
        st.markdown("**Absents récents**")
        st.info(", ".join(str(int(n)) for n in sorted(absents)))
    with col4:
        st.markdown("**Mix (3 top + 2 low)**")
        st.info(", ".join(str(int(n)) for n in sorted(mix)))
    ...
    def generate_combination():
        ...
        return sorted([int(b) for b in boules_choisies]), numero_chance
